...
def fetch_questions(category, difficulty, question_type, num_questions=10):
    ...
    if response.status_code == 200:
        ...
        if data["response_code"] == 0:  # Check if questions were returned successfully
            ...
            st.error(
                "Not enough questions available for the selected options.")  # Show an error if no questions are found
...
def main():
    ...
    if st.session_state.questions:
        ...
        st.progress((st.session_state.current_question - 0) / len(st.session_state.questions))
        ...
        if submit_button and not st.session_state.submitted:
            ...
            st.rerun() # butonul submit sa fie disabled din prima
        ...
        if st.session_state.submitted:
            ...
            if next_button:
                ...
                if st.session_state.current_question < len(st.session_state.questions):
                    ...
                    st.rerun() # butonul next sa functioneze din prima
                else:
